...
for hemi = 1:2
    ...
    for s = 1:length(no_space_list)
        ...
        save(fullfile(coupling_dir, [char(subject),'-',hemis{hemi},...
...
corr_filename = (['k-',num2str(k), '-fwhm-',num2str(fwhm) '.' layer{l} '2' ...
    dwi_metric{dwi} '-' surf_metric{sm} '-corrcoef.mat']);
save(fullfile(coupling_dir, corr_filename), 'all_corr_values','-v7');

p_filename = (['k-',num2str(k), '-fwhm-',num2str(fwhm) '.' layer{l} '2' ...
    dwi_metric{dwi} '-' surf_metric{sm} '-p.mat']);
save(fullfile(coupling_dir, p_filename), 'all_p_values','-v7');
...
for g = 1:length(age)
   ...
   %Pre-FDR correction
    ...
    colormap(RWB)
    SurfStatColLim([-1, 1])
    exportgraphics(gcf, strcat([coupling_dir 'images/' num2str(age(g))...
        '_' layer{l} '2' dwi_metric{dwi} '_vs_' surf_metric{sm} '_average_corrcoef_prefdr_map.jpg']),'Resolution',300)
    hold off
   %Post-FDR correction
    ...
    SurfStatColLim([-1, 1])
    exportgraphics(gcf, strcat([coupling_dir 'images/' num2str(age(g))...
        '_' layer{l} '2' dwi_metric{dwi} '_vs_' surf_metric{sm} '_average_corrcoef_postfdr_map.jpg']),'Resolution',300)
    hold off
end
